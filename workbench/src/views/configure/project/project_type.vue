<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加项目类型</el-button>
		<br>
		<el-dialog title="添加项目类型" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
...
<script>
  import { getProjectTypeList, addProjectType, editProjectType, delProjectType } from '@/api/project_type.js'
  ...
  export default {
	...
	methods:{
		...
		confirmEdit(data){
			...
			editProjectType(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delProjectType({id:id}).then( response => {	
					this.$message({
						message:'操作成功',
						type:'success'
					});
