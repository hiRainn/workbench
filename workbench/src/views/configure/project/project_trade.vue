<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加项目行业</el-button>
		<br>
		<el-dialog title="添加项目行业" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
		  <el-form :model="form" label-width="80px">
			<el-form-item label="行业名称">
			  <el-input v-model="form.trade_name" autocomplete="off" ></el-input>
			</el-form-item>
		  </el-form>
		...
		<el-table
			...
			<el-table-column
				...
				<template slot-scope="scope">
					<span v-if="!scope.row.edit">{{ scope.row.trade_name }}</span>
					<el-input size="small" v-if="scope.row.edit"  v-model="scope.row.trade_name" :value="scope.row.trade_name"/>
				</template>
			</el-table-column>
...
<script>
  import { getProjectTradeList, addProjectTrade, editProjectTrade, delProjectTrade } from '@/api/project_trade.js'
  ...
  export default {
	...
	methods:{
		...
		addcus(){
			...
			if(this.form.trade_name == false) {
				this.$alert('名称不能为空','提示');
				this.addDsiabled = false;
				return false
			}
			addProjectTrade(this.form).then( response => {
		...
		confirmEdit(data){
			data.disabled = true;
			if(data.trade_name == false) {
				this.$alert('名称不能为空','提示');
				data.disabled = false;
				return false
			}
			editProjectTrade(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delProjectTrade({id:id}).then( response => {
					this.$message({
						message:'操作成功',
						type:'success'
					});
