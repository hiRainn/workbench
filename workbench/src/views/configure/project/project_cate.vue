<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加项目类别</el-button>
		<br>
		<el-dialog title="添加项目类别" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
...
<script>
  import { getProjectCateList, addProjectCate, editProjectCate, delProjectCate } from '@/api/project_cate.js'
  ...
  export default {
	...
	methods:{
		...
		confirmEdit(data){
			...
			editProjectCate(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类别？', '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				delProjectCate({id:id}).then( response => {	
					this.$message({
						message:'操作成功',
						type:'success'
					});
