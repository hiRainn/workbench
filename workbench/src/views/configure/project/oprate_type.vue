<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加合作类型</el-button>
		<br>
		<el-dialog title="添加合作类型" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
...
<script>
  import { getOprateTypeList, addOprateType, editOprateType, delOprateType } from '@/api/oprate_type.js'
  ...
  export default {
	...
	methods:{
		...
		addcus(){
			...
			addOprateType(this.form).then( response => {
		...
		confirmEdit(data){
			...
			editOprateType(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delOprateType({id:id}).then( response => {	
					this.$message({
						message:'操作成功',
						type:'success'
					});
