<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加设备种类</el-button>
		<br>
		<el-dialog title="添加设备种类" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
		  <el-form :model="form" label-width="80px">
			<el-form-item label="设备名称">
			  <el-input v-model="form.kind_name" autocomplete="off" ></el-input>
			</el-form-item>
		  </el-form>
		...
		<el-table
			...
			<el-table-column
				...
				<template slot-scope="scope">
					...
					<el-input size="small" v-if="scope.row.edit"  v-model="scope.row.kind_name" :value="scope.row.kind_name"/>
				</template>
			</el-table-column>
...
<script>
  import { getDeviceKindList, addDeviceKind, editDeviceKind, delDeviceKind } from '@/api/device_kind'
  ...
  export default {
	...
	methods:{
		...
		addcus(){
			...
			if(this.form.kind_name == false) {
				this.$alert('名称不能为空','提示');
				this.addDsiabled = false;
				return false
			}
			addDeviceKind(this.form).then( response => {
		...
		confirmEdit(data){
			data.disabled = true;
			if(data.kind_name == false) {
				this.$alert('名称不能为空','提示');
				data.disabled = false;
				return false
			}
			editDeviceKind(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delDeviceKind({id:id}).then( response => {
					this.$message({
						message:'操作成功',
						type:'success'
					});
