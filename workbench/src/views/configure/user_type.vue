<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加类型</el-button>
		<br>
		<el-dialog title="添加用户类型" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
		...
		<el-table
			...
			<el-table-column
				...
				<template slot-scope="scope">
					<span v-if="!scope.row.edit">{{ scope.row.attr_name }}</span>
					<el-input size="small" v-if="scope.row.edit"  v-model="scope.row.attr_name" :value="scope.row.attr_name"/>
				</template>
			</el-table-column>
...
<script>
  import { getUserTypeList, addUserType, editUserType, delUserType } from '@/api/user_type.js'
  ...
  export default {
	...
	methods:{
		...
		addcus(){
			...
			if(this.form.attr_name == false) {
				this.$alert('名称不能为空','提示');
				this.addDsiabled = false;
				return false
			}
			addUserType(this.form).then( response => {
		...
		confirmEdit(data){
			data.disabled = true;
			if(data.attr_name == false) {
				this.$alert('名称不能为空','提示');
				data.disabled = false;
				return false
			}
			editUserType(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delUserType({id:id}).then( response => {	
					this.$message({
						message:'操作成功',
						type:'success'
					});
		...
		getList(data){
			this.loading = true
			getUserTypeList(data).then( response => {
