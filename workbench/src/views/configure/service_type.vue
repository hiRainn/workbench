<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加服务类型</el-button>
		<br>
		<el-dialog title="添加服务类型" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
...
<script>
  import { getServiceTypeList, addServiceType, editServiceType, delServiceType } from '@/api/service_type'
  ...
  export default {
	...
	methods:{
		...
		confirmEdit(data){
			...
			editServiceType(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delServiceType({id:id}).then( response => {	
					this.$message({
						message:'操作成功',
						type:'success'
					});
