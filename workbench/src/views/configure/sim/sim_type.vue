<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加号码类型</el-button>
		<br>
		<el-dialog title="添加号码类型" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
...
<script>
  import { getSimTypeList, addSimType, editSimType, delSimType } from '@/api/sim_type'
  ...
  export default {
	...
	methods:{
		...
		addcus(){
			...
			addSimType(this.form).then( response => {
		...
		confirmEdit(data){
			...
			editSimType(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delSimType({id:id}).then( response => {
					this.$message({
						message:'操作成功',
						type:'success'
					});
