...
<script>
  import { getSimStatusList, addSimStatus, editSimStatus, delSimStatus } from '@/api/sim_status'
  ...
  export default {
	...
	methods:{
		...
		addcus(){
			...
			addSimStatus(this.form).then( response => {
		...
		confirmEdit(data){
			...
			editSimStatus(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delSimStatus({id:id}).then( response => {
					this.$message({
						message:'操作成功',
						type:'success'
					});
