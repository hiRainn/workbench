<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加号码状态</el-button>
		<br>
		<el-dialog title="添加号码状态" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
...
<script>
  import { getSimUseStatusList, addSimUseStatus, editSimUseStatus, delSimUseStatus } from '@/api/sim_use_status'
  ...
  export default {
	...
	methods:{
		...
		addcus(){
			...
			addSimUseStatus(this.form).then( response => {
		...
		confirmEdit(data){
			...
			editSimUseStatus(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delSimUseStatus({id:id}).then( response => {
					this.$message({
						message:'操作成功',
						type:'success'
					});
    ...
    this.$notify({
      title: '提示',
      message: '号码使用状态，例如开通、欠费、半停机、欠费停机等'
    });
	}
  }
</script>
