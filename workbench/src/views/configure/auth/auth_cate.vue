<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加授权类别</el-button>
		<br>
		<el-dialog title="添加授权类别" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
		...
		<el-table
			...
			<el-table-column
				...
				<template slot-scope="scope">
					...
					<el-input size="small" v-if="scope.row.edit"  v-model="scope.row.cate_name" :value="scope.row.cate_name"/>
				</template>
			</el-table-column>
...
<script>
  import { getAuthCateList, addAuthCate, editAuthCate, delAuthCate } from '@/api/auth_cate'
  ...
  export default {
	...
	methods:{
		...
		addcus(){
			...
			if(this.form.cate_name == false) {
				this.$alert('名称不能为空','提示');
				this.addDsiabled = false;
				return false
			}
			addAuthCate(this.form).then( response => {
		...
		confirmEdit(data){
			...
			editAuthCate(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delAuthCate({id:id}).then( response => {
					this.$message({
						message:'操作成功',
						type:'success'
					});
