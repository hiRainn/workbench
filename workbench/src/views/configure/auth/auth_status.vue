<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加授权状态</el-button>
		<br>
		<el-dialog title="添加授权状态" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
...
<script>
  import { getAuthStatusList, addAuthStatus, editAuthStatus, delAuthStatus } from '@/api/auth_status'
  ...
  export default {
	...
	methods:{
		...
		addcus(){
			...
			addAuthStatus(this.form).then( response => {
		...
		confirmEdit(data){
			...
			editAuthStatus(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delAuthStatus({id:id}).then( response => {
					this.$message({
						message:'操作成功',
						type:'success'
					});
