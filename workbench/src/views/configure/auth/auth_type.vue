<template>
	<div class="app-container">
		...
		<el-button type="primary" @click="dialogFormVisible = true" style="float:right;margin-bottom: 10px;">添加授权类型</el-button>
		<br>
		<el-dialog title="添加授权类型" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
...
<script>
  import { getAuthTypeList, addAuthType, editAuthType, delAuthType } from '@/api/auth_type'
  ...
  export default {
	...
	methods:{
		...
		addcus(){
			...
			addAuthType(this.form).then( response => {
		...
		confirmEdit(data){
			...
			editAuthType(data).then( response => {
				data.edit=false;
				this.$message({
					message:'操作成功',
					type:'success'
		...
		delcus(id) {
			this.$confirm('是否确定删除该类型？', '提示', {
				...
				delAuthType({id:id}).then( response => {
					this.$message({
						message:'操作成功',
						type:'success'
					});
    ...
    this.$notify({
      title: '提示',
      message: '授权类型，例如普通用户、调度员、视频融合通信等'
    });
	}
  }
</script>
